...
typedef struct NohOperacao{
    ...
    char *quantidade; // tudo vai ser armazenado como string até o momento de fazer as operações matemática, a partir daí fazemos as conversões e desconversões
    char *valor;
}NohOperacao;

typedef struct Titulo{
    char* sigla; // const char* é utilizado para que a string possa ser retornada de uma função
...
void salvarOferta(char *sigla, char *tipo, char *valor, char *quantidade){
